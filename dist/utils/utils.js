...
export function findGroups(stringToLookIn, startToken, endToken) {
    const matches = [];
 ...
 */
export function getBracketLinks(text, skipIfNotDirect) {
    const links = [];
    let state = 'normal';
    let counter = 0;
    const currentLinks = [];
...
export function markdownToHTML(nodeContent) {
    if (!nodeContent) {
        return nodeContent;
    }
...
export function replaceTokenWithHtml(nodecontent, token, tagName) {
