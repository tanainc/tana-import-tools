import { findGroups, getBracketLinks, getCodeIfCodeblock, idgenerator, isIndexWithinBackticks, markdownToHTML, } from '../../utils/utils.js';
export class MarkdownConverter {
    constructor(fileSystem, pathIsh, fileToUrlMap) {
        this.nodesForImport = new Map();
        this.originalNodeNames = new Map();
        this.attrMap = new Map();
        this.topLevelMap = new Map();
        this.mdPathToPageUid = new Map();
        this.normalizedPathToUid = new Map();
        this.dirToBasenameToUid = new Map();
        this.pageUidToBaseDir = new Map();
        this.pageUidToMarkdownLinkNodeUids = new Map();
        this.csvTablesByParentAndPath = new Map();
    ...
    convertDirectory(dirPath) {
        ...
        const pageNodesByDepth = new Map();
        for (const f of files) {
            ...
            if (page) {
                ...
                const depthForFile = (_a = f.depth) !== null && _a !== void 0 ? _a : 0;
    ...
    postProcessAllNodes(rootLevelNodes) {
        const rootLevelNameToUid = new Map();
        ...
        for (const [, node] of this.nodesForImport) {
            ...
            if (newNodes === null || newNodes === void 0 ? void 0 : newNodes.length) {
                ...
                if (typeof child.name === 'string') {
                    ...
                    if (links.length > 0) {
                        ...
                        if (onlySeparatorsLeft) {
                            ...
                            const newChildren = links.map((l) => this.createNodeForImport({
                                uid: idgenerator(),
                                name: `[[${l}]]`,
    ...
    recordMarkdownLinkOccurrence(pageUid, nodeUid) {
    ...
    parseMarkdownLinks(text, callback) {
        let result = '';
        let i = 0;
        while (i < text.length) {
    ...
    convertRelativeLinksRecursively(node, baseDir, depth = 0) {
        if (node.type !== 'codeblock' && typeof node.name === 'string') {
            // 1) Convert standard markdown links [alias](relative)
            if (node.name.includes('](')) {
                node.name = this.parseMarkdownLinks(node.name, (alias, link) => {
                    var _a;
                    ...
                    const mappedUrl = (_a = this.fileToUrlMap) === null || _a === void 0 ? void 0 : _a.get(abs);
    ...
    inlineSinglyLinkedMarkdownPages(rootLevelNodes, pageNodesByDepth) {
        var _a, _b;
        ...
        const collectRefLocations = (current, parent) => {
        ...
        for (const [pageUid, nodeUids] of this.pageUidToMarkdownLinkNodeUids.entries()) {
            ...
            if ((((_a = refEntry.node.refs) === null || _a === void 0 ? void 0 : _a.length) || 0) > 1) {
                continue;
            }
            ...
            (_b = refEntry.parent.children) === null || _b === void 0 ? void 0 : _b.splice(refEntry.index, 1, pageNode);
    ...
    ensureCsvTableForLink(parent, fileDir, link, alias) {
        try {
            const decoded = this.safeDecode(link);
            ...
            const aliasDisplayName = alias === null || alias === void 0 ? void 0 : alias.trim();
    ...
    convertSingleFile(file) {
        ...
        if (((_a = lines[i]) === null || _a === void 0 ? void 0 : _a.trim()) === '---') {
            i++;
            ...
            while (i < lines.length && lines[i].trim() !== '---') {
                ...
                if (kv) {
                    ...
                    fm[currentKey].push(l.replace(/^\s*-\s+/, '').trim());
                }
                i++;
            }
            // consume closing ---
            ...
            for (const [key, val] of Object.entries(fm)) {
                ...
                if (Array.isArray(val)) {
                    for (const v of val) {
                        children.push(this.createNodeForImport({
                            uid: idgenerator(),
                            name: v,
        ...
        while (i < lines.length) {
            ...
            if (type !== 'codeblock' && paragraph.includes('](')) {
                ...
                paragraph = this.parseMarkdownLinks(paragraph, (alias, link) => {
    ...
    normalizeImageUrl(link, baseDir) {
        var _a;
        ...
        const mappedUrl = (_a = this.fileToUrlMap) === null || _a === void 0 ? void 0 : _a.get(absDecoded);
    ...
    normalizeCsvCellValue(value, sourceDir) {
    ...
    resolveUidForLinkedPath(linkTarget, sourceDir) {
    ...
    findExistingPageUidForCsvRow(rowDisplayName, mdSiblingDir) {
    ...
    stripQueryAndFragment(pathLike) {
        return pathLike.replace(/[?#].*$/, '');
    }
    ensureMarkdownExtension(pathLike) {
    ...
    normalizePathForComparison(pathLike) {
    ...
    resolvePendingCsvCellReferences() {
    ...
    findFileByBasename(dir, base, visited) {
    ...
    isInlineDateValue(text) {
        ...
        const shortMonth = /^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)\.?\s+\d{1,2},\s*\d{4}(?:\s+\d{1,2}:\d{2}(?::\d{2})?\s*(?:AM|PM)?)?$/i;
    ...
    convertFieldLine(nodeWithField, parentNode) {
        ...
        let currentFieldNode = nodeWithField;
        for (const field of fieldDefs) {
            ...
            if (links.length > 0 && onlySeparatorsLeft) {
                for (const l of links) {
                    values.push(this.createNodeForImport({
                        uid: idgenerator(),
                        name: `[[${l}]]`,
    ...
    getValueForAttribute(fieldName, node) {
        if (!node.includes('::')) {
            return undefined;
        }
