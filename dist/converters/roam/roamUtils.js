...
export function setNodeAsTodo(node) {
    node.name = node.name.substring(node.name.substring(0, TODO_FLAG.length) === TODO_FLAG ? TODO_FLAG.length : TODO_FLAG_ALT.length);
    node.todoState = 'todo';
}
export function setNodeAsDone(node) {
    node.name = node.name.substring(node.name.substring(0, TODO_FLAG.length) === DONE_FLAG ? DONE_FLAG.length : DONE_FLAG_ALT.length);
    node.todoState = 'done';
}
export function replaceRoamSyntax(nameToUse) {
